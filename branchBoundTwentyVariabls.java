			...
			for (int j = 0; j < temp[0].length; j++) {
				...
				if (temp[i][j] >= 0) {
					if ((temp[i][j] - (int) temp[i][j] < 1e-9)) {
						...
						continue;
					} else if (((int) temp[i][j] + 1 - temp[i][j]) < 1e-9) {
					...
					if (temp[i][j] - (int) temp[i][j] + 1 < 1e-9) {
						temp[i][j] = (int) temp[i][j] - 1;
						continue;
					} else if (-temp[i][j] + (int) temp[i][j] < 1e-9) {
						temp[i][j] = (int) temp[i][j];
						continue;
					} else {
						loc[0] = i;
